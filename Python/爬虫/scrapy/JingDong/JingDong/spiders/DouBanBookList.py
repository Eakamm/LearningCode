...
class DouBanBookList(scrapy.Spider):
    ...
    def __init__(self, dou_ban_id=None, **kwargs):
        ...
        self.urls = [
            ...
            "https://book.douban.com/subject/%s" % dou_ban_id
        ]
    ...
    def parse_details(self, response):
        ...
        book['CommitNumbers'] = details.find('div','rating_sum').span.a.span.string.strip()
        ...
        if book['jd_url'] is None:
            url = 'https://search.jd.com/Search?keyword='+book['title']+'&shop=1&click=1'
            request = scrapy.Request(url=url, callback=self.jing_dong_search)
    ...
    def jing_dong_search(self, response):
        book = response.meta['book']

        soup = BeautifulSoup(response.text, 'html.parser')
        # 找到商品列表的div
        good_list = soup.find('div', id='J_goodsList')
        # 第一个商品的位置
        good = good_list.ul.li
        jd_url = 'https:'+good.find('a', target="_blank")['href']
